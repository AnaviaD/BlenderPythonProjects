...
from PyQt5.QtWidgets import (
    ...
    QApplication, QCheckBox, QDoubleSpinBox, QProgressBar, QDialog
)
...
class ClickWorker(QThread):
    ...
    def __init__(self, positions, base_cooldown, random_range=0.5):
        super().__init__()
...
class MainWindow(QMainWindow):
    ...
    def update_cooldown_label(self, state):
        """Actualiza el label del rango de cooldown"""
        base = self.cooldown_spinbox.value()
        min_val = 900
        if state == Qt.Checked:
            low = min(base, min_val)
            high = max(base, min_val)
            self.cooldown_range_label.setText(f"(rango: {low:.1f} - {high:.1f} ms)")
        else:
            self.cooldown_range_label.setText(f"(fijo: {base:.1f} ms)")
    ...
    def on_analysis_finished(self, positions: list):
        ...
        self.label_analysis_results.setText(f"✅ Encontrados: {count} objetivos únicos")
        self.label_analysis_results.setStyleSheet(
            "color: #4CAF50; padding: 5px; background-color: #f0f0f0; border-radius: 3px;"
        )
        ...
        if count > 0:
            QMessageBox.information(self, "Análisis Completado", 
                f"✅ Se encontraron {count} objetivos únicos.\n\nAhora puedes ejecutar los clics.")
        else:
            QMessageBox.information(self, "Análisis Completado", 
                f"⚠️ No se encontraron objetivos con el color seleccionado.")
    ...
    def setup_ui(self):
        ...
        # --- Checkbox para filtrar por tamaño ---
        self.filter_checkbox = QCheckBox("🔲 Filtrar por tamaño (objetos con marco)")
        self.filter_checkbox.setChecked(True)  # Activado por defecto
        self.filter_checkbox.stateChanged.connect(self.toggle_size_filter)
        group3_layout.addWidget(self.filter_checkbox)
        ...
        # --- Control de cooldown con aleatorización ---
        cooldown_layout = QHBoxLayout()
        
        cooldown_layout.addWidget(QLabel("⚡ Cooldown (ms):"))
        
        self.cooldown_spinbox = QDoubleSpinBox()
        self.cooldown_spinbox.setRange(1, 500)
        self.cooldown_spinbox.setValue(150.5)
        self.cooldown_spinbox.setSingleStep(0.5)
        cooldown_layout.addWidget(self.cooldown_spinbox)
        
        self.random_cooldown_checkbox = QCheckBox("🎲 Aleatorizar")
        self.random_cooldown_checkbox.setChecked(False)
        cooldown_layout.addWidget(self.random_cooldown_checkbox)
        
        self.cooldown_range_label = QLabel("(fijo: 15.5 ms)")
        cooldown_layout.addWidget(self.cooldown_range_label)
        
        # Conectar el checkbox para actualizar el label
        self.random_cooldown_checkbox.stateChanged.connect(self.update_cooldown_label)
        
        group4_layout.addLayout(cooldown_layout)
    ...
    def toggle_size_filter(self, state):
        """Activa/desactiva el filtro por tamaño"""
        enabled = state == Qt.Checked
        self.color_matcher.filter_by_size = enabled
        self.log_info(f"🔲 Filtro por tamaño: {'Activado' if enabled else 'Desactivado'}")
    ...
    def select_color(self):
        ...
        try:
            ...
            # Guardar color con el tamaño
            self.target_color = color_rgb
            self.color_matcher.set_target_color(color_rgb, sample_size)  # <--- AHORA CON TAMAÑO
    ...
    def execute_clicks(self):
        """Ejecuta los clics en las posiciones encontradas con cooldown aleatorio"""
        try:
            ...
            # Obtener cooldown base del spinbox
            base_cooldown = self.cooldown_spinbox.value()
            
            # Verificar si está aleatorizado
            if self.random_cooldown_checkbox.isChecked():
                random_range = 0.5  # ±50% de variación
                time_est = count * base_cooldown / 1000 * 1.5  # Estimación con aleatoriedad
                self.log_info(f"🎲 Cooldown aleatorio ±50% (base: {base_cooldown:.1f}ms)")
            else:
                random_range = 0.0  # Sin aleatoriedad
                time_est = count * base_cooldown / 1000
            ...
            reply = QMessageBox.question(
                ...
                f"⚡ Cooldown base: {base_cooldown:.1f}ms\n"
                f"🎲 Aleatorización: {'Sí (±50%)' if random_range > 0 else 'No'}\n"
                f"⏱️ Tiempo estimado: {time_est:.2f} segundos\n"
            ...
            # Crear worker con cooldown y aleatoriedad
